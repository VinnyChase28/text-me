...
export const quoteCronRouter: any = createRouter().mutation("new-quote-cron", {
  ...
  resolve: async ({ input }) => {
    ...
    const cronEndpoint = `${baseUrls.cronAdd}?token=${easy_cron_token}&cron_expression=${cronExpression}&url=${baseUrls.newQuoteCron}&http_method=post&http_message_body=${body}&timezone_from=2&timezone=${timezone}&http_headers=Content-Type:application%2Fjson&cron_job_name=${input?.phone}`;
